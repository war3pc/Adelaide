...
fileprivate extension GXCardCell {
    ...
    func endCenter(direction: SwipeDirection, view: UIView) -> CGPoint {
        ...
        let centerX: CGFloat = view.center.x + rect.origin.x - view.frame.origin.x
        ...
        case .right:
            ...
            return CGPoint(x: endCenterX, y: centerY)
        case .top:
            let endCenterY = -(GX_ScreenHeight*0.5 + self.frame.height)
            return CGPoint(x: centerX, y: endCenterY)
        default: return .zero
        }
    }
    func didPanStateChanged(move point: CGPoint) {
        ...
        if (self.currentPoint.y < -self.maxRemoveDistance) {
            self.delegate?.cardCell(self, didMoveAt: self.currentPoint, direction: .top)
        }
    ...
    func didPanStateEnded() {
        ...
        if (self.currentPoint.y < -self.maxRemoveDistance) {
            self.remove(direction: .top, isPan: true)
        }
        else {
            self.restorePosition()
        }
